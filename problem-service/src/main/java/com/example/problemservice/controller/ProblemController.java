...
public class ProblemController {
  ...
  public ResponseEntity<DetailsProblemResponse> getProblem(
    ...
    try {
      ...
      if (e.getErrorCode() == ErrorCode.PROBLEM_NOT_EXIST) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null); // Return 404 if problem
                                                                       // not found
      }
      throw e;
    }
  ...
  @GetMapping("/search")
  public ApiResponse<Page<ProblemRowResponse>> getProblems(
      @RequestParam(required = false) List<Integer> categories,
      @RequestParam(required = false) String level,
  ...
  public ResponseEntity<ProblemCreationResponse> updateProblem(@PathVariable UUID problemId,
      @RequestBody ProblemCreationRequest request) {
    return ResponseEntity.ok(problemService.updateProblem(problemId, request));
  }

  @Operation(summary = "(testing only) Generate boilerplate code")
  ...
  public ApiResponse<List<DefaultCodeResponse>> generateBoilerPlate(@RequestBody DefaultCodeRequest request) {
    return ApiResponse.<List<DefaultCodeResponse>>builder()
        .result(problemService.generateDefaultCodes(request.getProblemId(),
  ...
  // api để test code
  @Operation(summary = "(testing only) Enrich code"
  // ,hidden = true
  )
  ...
  @Operation(summary = "Get all solution of a problem")
  ...
  @Operation(summary = "(testing only) Get problem by id in file")
  ...
  @Operation(summary = "Get partial boilerplate of problem by id")
  ...
  @Operation(summary = "Get all problem comment by problem id")
